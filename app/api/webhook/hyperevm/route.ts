...
export async function POST(request: NextRequest) {
  try {
    ...
    const webhookSecret = process.env.ALCHEMY_HYPEREVM_SIGNING_KEY || process.env.ALCHEMY_WEBHOOK_SIGNING_KEY || '';
    ...
    console.log('HyperEVM webhook received:', JSON.stringify(payload, null, 2));
    ...
    console.error('HyperEVM webhook error:', error);
...
async function processPayment(payment: any): Promise<void> {
  try {
    ...
    const isConfirmed = await verifyPayment(payment, 'hyperevm');
    if (!isConfirmed) {
