...
export async function POST(request: NextRequest) {
  try {
    ...
    const webhookSecret = process.env.ALCHEMY_ARB_SIGNING_KEY || process.env.ALCHEMY_WEBHOOK_SIGNING_KEY || '';
    ...
    console.error('Arbitrum webhook error:', error);
...
async function processPayment(payment: any): Promise<void> {
  try {
    ...
    // Verify confirmations (ARB temporarily disabled)
    // const isConfirmed = await verifyPayment(payment, 'arb');
    const isConfirmed = true; // Skip verification for disabled ARB
