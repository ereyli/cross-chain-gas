...
export async function POST(request: NextRequest) {
  try {
    ...
    const webhookSecret = process.env.ALCHEMY_ABSTRACT_SIGNING_KEY || process.env.ALCHEMY_WEBHOOK_SIGNING_KEY || '';
    ...
    console.log('Abstract webhook received:', JSON.stringify(payload, null, 2));
    ...
    console.error('Abstract webhook error:', error);
...
async function processPayment(payment: any): Promise<void> {
  try {
    ...
    const isConfirmed = await verifyPayment(payment, 'abstract');
    if (!isConfirmed) {
