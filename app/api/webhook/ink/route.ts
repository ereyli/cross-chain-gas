...
export async function POST(request: NextRequest) {
  try {
    ...
    const webhookSecret = process.env.ALCHEMY_INK_SIGNING_KEY || process.env.ALCHEMY_WEBHOOK_SIGNING_KEY || '';
    ...
    console.log('Ink webhook received:', JSON.stringify(payload, null, 2));
    ...
    console.error('Ink webhook error:', error);
...
async function processPayment(payment: any): Promise<void> {
  try {
    ...
    const isConfirmed = await verifyPayment(payment, 'ink');
