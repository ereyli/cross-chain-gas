...
export async function POST(request: NextRequest) {
  try {
    ...
    const webhookSecret = process.env.ALCHEMY_ETH_SIGNING_KEY || process.env.ALCHEMY_WEBHOOK_SIGNING_KEY || '';
    ...
    console.log('Ethereum webhook received:', JSON.stringify(payload, null, 2));
...
async function processPayment(payment: any): Promise<void> {
  try {
    ...
    const isConfirmed = await verifyPayment(payment, 'eth');
