...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Optimism webhook received:', JSON.stringify(payload, null, 2));
    ...
    console.error('Optimism webhook error:', error);
...
async function processPayment(payment: any): Promise<void> {
  try {
    ...
    const isConfirmed = await verifyPayment(payment, 'op');
